...
state['EuXFEL/DataSource'] = 'tcp://exflonc02.desy.de:1234' # Raw
...
num_cell = 64  # Number of AGIPD mem cells
...
# Pulse filter
# base_pulse_filter = np.zeros(num_cell, dtype="bool")
...
base_initmask = np.ones((128,512,num_cell)).astype(np.bool)
if usemask:
...
def onEvent(evt):
    ...
    analysis.event.printProcessingRate(pulses_per_event=num_cell, label="Processing rate (pulses):" )
